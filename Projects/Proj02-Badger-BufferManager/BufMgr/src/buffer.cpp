...
BufMgr::~BufMgr() { delete[] bufPool; }

void BufMgr::advanceClock() { clockHand = (clockHand + 1) % numBufs; }

void BufMgr::allocBuf(FrameId &frame) {

  // the worst case, we need to run two cycles to ensure all frames are
  // unavalible
...
void BufMgr::allocPage(File *file, PageId &pageNo, Page *&page) {
  FrameId frameNo = 0;
  // prepare a buffer pool frame for the new allocate page
  allocBuf(frameNo);

  // allocate an empty page for the speicfic file and assign it to the reserved
  // farme
